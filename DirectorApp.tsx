...
import { DirectorQAPanel } from './components/DirectorQAPanel';
...
const App: React.FC = () => {
  ...
  const [showDirectorQA, setShowDirectorQA] = useState(true);
  ...
  return (
    <div className="h-screen relative flex flex-col overflow-hidden selection:bg-white/20 bg-[#0d0d0d]">
      ...
      {/* Director Mode Badge */}
      <div className="fixed top-4 right-4 z-[100] bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg flex items-center gap-2">
        <span>🎬</span>
        <span>Director Mode (Experimental)</span>
      </div>
      ...
      {/* Director QA Panel - shows performance validation */}
      {data?.directorQA && showDirectorQA && (
        <DirectorQAPanel 
          qaReport={data.directorQA}
          onClose={() => setShowDirectorQA(false)}
        />
      )}
    </div>
  );
